...
if ($FORM['dohal'] == 'clear') {
    redirpageto('index.php?hal=saleslist');
    exit;
}
...
//Main queries
$sql = $db->getRecFrmQry("SELECT * FROM " . DB_TBLPREFIX . "_sales LEFT JOIN " . DB_TBLPREFIX . "_items ON slitid = itid LEFT JOIN " . DB_TBLPREFIX . "_mbrs ON itidmbr = id WHERE 1 " . $condition . "");
$pages->items_total = count($sql);
$pages->mid_range = 3;
$pages->paginate();

$salesData = $db->getRecFrmQry("SELECT * FROM " . DB_TBLPREFIX . "_sales LEFT JOIN " . DB_TBLPREFIX . "_items ON slitid = itid LEFT JOIN " . DB_TBLPREFIX . "_mbrs ON itidmbr = id WHERE 1 " . $condition . $sqlshort . $pages->limit . "");
?>

<div class="section-header">
    <h1><i class="fa fa-fw fa-store"></i> <?php echo myvalidate($LANG['a_saleslist']); ?></h1>
</div>
...
<div class="section-body">
    ...
    <form method="get">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="float-md-right">
                            <a href="index.php?hal=saleslist&dohal=clear" class="btn btn-warning"><i class="fa fa-fw fa-redo"></i> Clear</a>
        ...
        <input type="hidden" name="hal" value="saleslist">
    </form>
                ...
                if (count($salesData) > 0) {
                    ...
                    foreach ($salesData as $val) {
                        ...
                        <tr>
                            ...
                            <td><?php echo myvalidate($val['itname']); ?></td>
                            ...
                            <td align="center" nowrap>
                                <a href="javascript:;"
                                   ...
                                   if ($val['username'] != '') {
                                       ?>
                                       <div class='text-small mt-2'><?php echo myvalidate($LANG['g_vendor']); ?>:</div>
                                       <div class='text-info'><?php echo myvalidate($val['username']); ?></div>
                                       <?php
                                   }
                                   ?>
                                   <div class='text-small mt-2'><?php echo myvalidate($LANG['g_buyer']); ?>:</div>
                                   <div><strong><?php echo myvalidate($val['slmbrun']); ?></strong></div>
                                ...
                                <a href="javascript:;" data-href="dosales.php?editId=<?php echo myvalidate($val['slid']); ?>&redir=saleslist&mdlhasher=<?php echo myvalidate($mdlhasher); ?>" data-poptitle="<i class='fa fa-fw fa-edit'></i> Update Sales History #<?php echo myvalidate($val['slid']); ?>" class="btn btn-sm btn-success openPopup" data-toggle="tooltip" title="Update <?php echo myvalidate($val['slbatch']); ?>"><i class="fa fa-fw fa-edit"></i></a>
                                <a href="javascript:;" data-href="dosales.php?hash=<?php echo myvalidate($hasdel); ?>&delId=<?php echo myvalidate($val['slid']); ?>&redir=saleslist&mdlhasher=<?php echo myvalidate($mdlhasher); ?>" class="btn btn-sm btn-danger bootboxconfirm" data-poptitle="Transaction ID: <?php echo myvalidate($val['slid'] . '-' . $val['slbatch']); ?>" data-popmsg="Are you sure want to delete this sales history?" data-toggle="tooltip" title="Delete <?php echo myvalidate($val['slbatch']); ?>"><i class="far fa-fw fa-trash-alt"></i></a>
                            </td>
