...
if ($_SESSION['mbrfltrby'] != '') {
    $condition .= " AND mpid < '1'";
    $joinersql = "LEFT JOIN";
    $mbrfltrbystr = 'Registered Only';
    $mbrfltrbyinfo = 'Display registered only';
} else {
    $joinersql = "JOIN";
    $mbrfltrbystr = 'Member Only';
    $mbrfltrbyinfo = 'Display registered and join to the membership';
}

//Main queries
$sql = $db->getRecFrmQry("SELECT * FROM " . DB_TBLPREFIX . "_mbrs {$joinersql} " . DB_TBLPREFIX . "_mbrplans ON id = idmbr WHERE 1 " . $condition . "");
$pages->items_total = count($sql);
$pages->mid_range = 3;
$pages->paginate();

$userData = $db->getRecFrmQry("SELECT * FROM " . DB_TBLPREFIX . "_mbrs AS mbr {$joinersql} " . DB_TBLPREFIX . "_mbrplans AS plan ON id = idmbr LEFT JOIN " . DB_TBLPREFIX . "_sessions AS ses ON mbr.id = ses.sesidmbr WHERE 1 " . $condition . ' GROUP BY mpid ' . $sqlshort . $pages->limit . "");
?>
                        ...
                        <div class="float-md-right">
                            <div class="btn-group">
                                ...
                                <div class="dropdown-menu">
                                    <a class="dropdown-item" href="index.php?hal=userlist&mbrfltrby=mbr"><i class="fa fa-user-tie fa-fw"></i> Member</a>
                                    <a class="dropdown-item" href="index.php?hal=userlist&mbrfltrby=reg"><i class="fa fa-user fa-fw"></i> Register Only</a>
                                </div>
                            </div>
