...
if ($FORM['stripe_txmpid'] != '') {
    try {
        ...
        $FORM['sb_type'] = 'payreg';
        doipnbox($_POST['stripe_txmpid'], $amountreal, 'stripeacc', $paidbatch, '', 'continue', 0, '');
    ...
    redirpageto($cfgrow['site_url'] . '/' . MBRFOLDER_NAME . '/index.php?hal=planpay&stripe_form=stripe_txmpid');
    ...
    try {
        ...
        if (empty($api_error) && $checkout_session) {
            ...
            if ($checkout_session->payment_status == 'paid') {
                ...
                include_once('../common/sandbox.php');
                $FORM['sb_type'] = 'payreg';
                doipnbox($client_reference_id, $paidAmount, 'stripeacc', $transactionID, '', 'continue', 0, '');
            } else {
    ...
    redirpageto($cfgrow['site_url'] . '/' . MBRFOLDER_NAME . '/index.php?hal=planpay&stripe_form=session_id');
}
