...
$mbrrazorpaycfg = get_optarr($pgmbrtokenarr['razorpaycfg']);
...
$regfee = $totcoinpayments = $totstripe = $totperfectmoney = $totpaypal = $totpayfast = $totrazorpay = $totpaystack = $totewallet = $totmanualpay = $tottestpay = $paynow_fee;

$isppsandbox = $paypalcfg['paypalsbox'];

$ispayg = 0;
$paygatearr = array('coinpayments', 'perfectmoney', 'paypal', 'stripe', 'payfast', 'razorpay', 'paystack', 'ewallet', 'manualpay', 'testpay');
...
$manualpayipn = strtr($manualpayipn, $manualpaytagsarr);
$manualpayipn64 = base64_encode($manualpayipn . '<hr /><button type="button" class="btn btn-warning btn-lg mt-2" onclick="location.href = \'index.php?hal=feedback&ispaidconfirm=' . base64_encode($txmpid) . '&pby=manualpayipn\'">Confirm Payment</button>');
?>
...
<div class="section-body">
    ...
    <div class="row">
        ...
        if ($pgdatatokenarr['razorpayon'] == 1) {

            $posturl = "../common/razorpay.do.php";
            ?>
            <div class="<?php echo myvalidate($colmdclass); ?>">
                <div class="card card-primary">
                    <div class="card-body text-center">
                        <?php echo myvalidate($avalpaygateicon_array['razorpaykeyid']); ?>
                        <h4>RazorPay</h4>
                        <div class="mt-4"><?php echo myvalidate($LANG['g_amount']); ?>: <?php echo myvalidate($bpprow['currencysym'] . $regfee); ?></div>
                        <div><code><?php echo myvalidate($LANG['g_servicefee']); ?>: <?php echo myvalidate($bpprow['currencysym'] . $feerazorpay); ?></code></div>
                        <h6>Total: <?php echo myvalidate($bpprow['currencysym'] . $totrazorpay . ' ' . $bpprow['currencycode']); ?></h6>
                        <form method="post" action="<?php echo myvalidate($posturl); ?>" id="dopayform">
                            <input type="hidden" name="item_name" value="<?php echo myvalidate($bpprow['ppname']); ?>">
                            <input type="hidden" name="item_description" value="<?php echo myvalidate($mbrstr['username']); ?>">
                            <input type="hidden" name="item_number" value="<?php echo myvalidate($txmpid); ?>">
                            <input type="hidden" name="amount" value="<?php echo myvalidate($totrazorpay); ?>">
                            <input type="hidden" name="currency" value="<?php echo myvalidate($bpprow['currencycode']); ?>">
                            <input type="hidden" name="name" value="<?php echo myvalidate($mbrstr['fullname']); ?>">
                            <input type="hidden" name="email" value="<?php echo myvalidate($mbrstr['email']); ?>">
                            <input type="hidden" name="contact" value="<?php echo myvalidate($mbrstr['phone']); ?>">
                            <button type="submit" name="dopay" value="1" id="dopay" class="btn btn-primary btn-lg mt-4">
                                <?php echo myvalidate($LANG['m_makepayment']); ?>
                            </button>
                        </form>
